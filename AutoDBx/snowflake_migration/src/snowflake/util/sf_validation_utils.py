...
class Validation:
    def __init__(self, spark, config_catalog, config_schema, config_table, validation_catalog, validation_schema, validation_table, sfURL,sfUser,sfPassword, sfWarehouse, sfRole, src, jdbc_driver):
        self.spark = spark
    ...
    def read_data(self, source_table, sfOptions):
        """
        Read data from source database using JDBC connection
        """
        # jdbc_url = f"jdbc:{self.src}://{self.host}:{self.port}/{source_schema}"

        # print("Inside the read function")
        # print(sfOptions)
        # print(source_table)
    ...
    def validate_tables(self, target_table, source_table, sfOptions):
        """
        ...
        try:
            ...
            print(sfOptions)
            source_df = self.read_data(source_table_name,sfOptions)
    ...
    def main(self):
        ...
        config_df = self.spark.table(f"{self.config_catalog}.{self.config_schema}.{self.config_table}") \
                    ...
                    .select("source_schema", "source_table", "target_table_catalog", "target_table_schema", "target_table_name", "load_type", "key_column", "load_type_col_name", 'source_database')
        ...
        for row in config_df.collect():
            ...
            print(target_table)
            print(source_table)
            self.validate_tables(target_table, source_table, sfOptions=sfOptions)
