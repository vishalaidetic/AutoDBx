...
class Migration:
    def __init__(self,spark,config_catalog,config_schema,config_table, host,port, user_name, password,src,logger, jdbc_driver, hostNameInCertificate):
        self.spark = spark
    ...
    def migrate_incremental_data(self,source_schema, source_table, target_table_catalog, target_table_schema, target_table,incremental_col_name,PK, jdbc_driver, partition_by=None, cluster_by=None, optimize=None, z_order=None):
        try:
            ...
            query = f"SELECT * FROM {source_table} WHERE {incremental_col_name} > '{last_load_time}'"
